...
export function assistantSystemPrompt(delegates: readonly { key: string; title: string }[]): string {
  ...
  return [
    ...
    'actually has. One call per device — and when a request covers several devices, make every',
    'one of those calls in the same response, however many there are, rather than one device per',
    'round. They are carried out in the order you send them, and each extra round is time somebody',
    'spends waiting.',
    '',
