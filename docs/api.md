...
| `POST /assistant/voice/session` | `hub.ai` | open a live **voice** session: `{sessionId?}` → `201 {clientSecret, expiresAt, socketUrl, startFrame, audioRate, sessionId}`. The hub builds the whole GPT-Live session — instructions, voice, history, delegation mode — and answers with the `session.start` **frame already serialised**, so the phone forwards an opaque string and names no session field at all: a prompt change, a voice change or a new configuration key reaches the microphone with no app release. `socketUrl` is answered rather than pinned in the app for the same reason, and one more: it is the one thing on this surface nobody here can verify (see `src/ai/voice/live-wire.ts`). **The home’s OpenAI key never leaves the hub** — `clientSecret` is an `ek_…` value that expires. `delegation.type` is **`client`**, which is the whole architecture in one field: the voice asks *this hub* for help rather than a model OpenAI hosts, so the home keeps its own agent, its own tools, its own transcript and whichever provider it picked. `sessionId` is the assistant conversation the phone will write into — **sent back** when the app already has one, since stopping and restarting the microphone on a page is one conversation rather than two, and minted fresh when it is absent; when it is sent, that conversation’s last few exchanges are seeded into `session.input`, so pressing the microphone on a page you have been typing on continues one conversation. It grants nothing new: `voice/said` already writes against whatever id it is given, and a home’s transcripts are shared by design. Refuses `409 ai_disabled`, and `409 openai_not_configured` — a fourth code, and a real one: GPT-Live is OpenAI’s and there is no substitute, so a home whose assistant answers perfectly well on Anthropic still cannot *speak* without an OpenAI key. `502 voice_unavailable` when OpenAI itself refuses, with their sentence in `detail` — a different screen from a `409`, which is something in this home’s settings |
