...
| `POST /assistant/voice/session` | `hub.ai` | open a live **voice** session: `{sessionId?}` → `201 {clientSecret, expiresAt, model, audioRate, sessionId}`. The hub builds the whole GPT-Live session — instructions, tools, voice, audio formats — and mints an ephemeral client secret against it, so the phone composes nothing and **the home's OpenAI key never leaves the hub**: `clientSecret` is an `ek_…` value that expires. `sessionId` is the assistant conversation the phone will write into — **sent back** when the app already has one, since stopping and restarting the microphone on a page is one conversation rather than two, and minted fresh when it is absent. It grants nothing new: `voice/said` already writes against whatever id it is given, and a home's transcripts are shared by design. Refuses `409 ai_disabled`, and `409 openai_not_configured` — a fourth code, and a real one: GPT-Live is OpenAI's and there is no substitute, so a home whose assistant answers perfectly well on Anthropic still cannot *speak* without an OpenAI key. `502 voice_unavailable` when OpenAI itself refuses, with their sentence in `detail` — a different screen from a `409`, which is something in this home's settings |
| `POST /assistant/voice/tool` | `hub.ai` | run one of the voice's tools: `{sessionId, name, input?}` → `{text, detail?, isError?}`. The **fast half** of the arrangement — the phone proxies a tool call here in one LAN hop rather than waiting on a reasoning model, because switching a lamp should take a third of a second. The catalog is the assistant's own minus `ask_user` and `delegate`, and it runs in exactly the context a typed conversation uses: same registry path, same activity row, named for the same person, with `data.via: "voice"` the only thing that differs |
| `POST /assistant/voice/said` | `hub.ai` | write down what was said out loud: `{sessionId, role, text, steps?}` → `204`. The **same transcript the typed chat uses**, which is what makes the page fill in while somebody talks, be there when they open it afterwards, and be continuable by typing — `revive()` rebuilds a model conversation from exactly these rows. The client is the only thing that knows what was said, because the hub is not in the audio path; the row is written under the caller's own member id |
| `POST /assistant/voice/ended` | `hub.ai` | what a voice session cost: `{sessionId, seconds}` → `204`, written as an `ai_runs` row of `kind: "voice"` at $0.11 a minute. Its **own** row rather than an addition to the conversation's, because they are two meters: the voice layer bills for audio while the model behind it bills for tokens and has already written `assist` rows. **The per-minute figure is a bound rather than a measurement**, and the top of the band on purpose — the realtime models bill per audio token split by direction (~$0.019/min heard, ~$0.077/min spoken) and the phone reports only seconds on the line, so pricing the unsplit number at the dearer end is the portrait ledger's rule: an estimate that reads low is the one that surprises somebody. Silence still costs, since the microphone streams continuously. The seconds are the phone's measurement, which is softer than anything else in this ledger and is the only one available; a session that ends without this arriving records nothing rather than guessing |
