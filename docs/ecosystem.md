...
| `gethome-ios` | The GetHome iOS app — the daily driver for controlling homes. Supports three home types: an Apple Home mirror, a no-hub home (Matter devices commissioned directly onto the iPhone), and **hub homes** served by this project. |
...
The iOS app can commission Matter devices directly onto the phone — but then
the *phone* owns them: the fabric credentials live in that phone's Keychain
and cannot be shared. A hub owns its devices instead, so any phone with access
to the hub can control the home. **Hub homes are the only shareable kind of
GetHome home** — plus they add Zigbee (via a USB stick) and MQTT integrations,
which phones can't do at all.
