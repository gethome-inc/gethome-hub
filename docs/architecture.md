...
|---|---|
| `src/schema/` | The canonical device schema: 25 capability kinds, 16 device kinds, typed endpoint state, 17 command intents, unit conversions, Matter device-type catalog, zod wire schemas. Dependency-free (zod only) — everything else derives from it. |
